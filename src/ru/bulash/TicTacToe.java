...
public abstract class TicTacToe {
    ...
    private void init() {
        ...
        this.gameField = new char[this.dimension][this.dimension];
        ...
        this.winSequence = newWinSequence;
    ...
    protected void humanTurn() {
        ...
        do {
            ...
            valid = (row >= 0) && (row < this.dimension);
    ...
    protected boolean isWin(boolean human) {
        ...
        for (int diagonal = 0; diagonal < dimension; diagonal++) {
            for (int row = diagonal, column = 0; row < dimension; row++, column++) {
                if(gameField[row][column] == (human ? HUMAN_DOT : AI_DOT)) {
                    dSequence++;
                    if (dSequence == winSequence) {
        ...
        dSequence = 0;
        for (int diagonal = 1; diagonal < dimension; diagonal++) {
            for (int column = diagonal, row = 0; column < dimension; column++, row++) {
                if(gameField[row][column] == (human ? HUMAN_DOT : AI_DOT)) {
                    dSequence++;
                    if (dSequence == winSequence) {
                        return true;    // Победа по диагонали
        ...
        dSequence = 0;
        for (int diagonal = 1; diagonal < dimension; diagonal++) {
            for (int row = diagonal, column = dimension - 1; row < dimension; row++, column--) {
                if(gameField[row][column] == (human ? HUMAN_DOT : AI_DOT)) {
                    dSequence++;
                    if (dSequence == winSequence) {
                        return true;    // Победа по диагонали
    ...
    protected void dropWeights() {
        ...
        this.cellWeights = new int[this.dimension][this.dimension];
    ...
    protected void calcWeight(int y, int x) {
        ...
        // Восток (east)
        weight = 0;
        for (int column = x; column < dimension; column++) {
        ...
        // Юг (south)
        weight = 0;
        for (int row = y; row < dimension; row++) {
            if(this.gameField[row][x] == HUMAN_DOT) {
                weight++;
            } else if (this.setCellWeight(row, x, weight--)) {
                weight = 0;
        ...
        weight = 0;
        for (int row = y, column = x; row < dimension && column < dimension; row++, column++) {
            if(this.gameField[row][column] == HUMAN_DOT) {
                weight++;
            } else if (this.setCellWeight(row, column, weight--)) {
                weight = 0;
        ...
        weight = 0;
        for (int row = y, column = x; row >= 0 && column < dimension; row--, column++) {
            if(this.gameField[row][column] == HUMAN_DOT) {
                weight++;
            } else if (this.setCellWeight(row, column, weight--)) {
                weight = 0;
        ...
        weight = 0;
        for (int row = y, column = x; row < dimension && column >= 0; row++, column--) {
            if(this.gameField[row][column] == HUMAN_DOT) {
                weight++;
            } else if (this.setCellWeight(row, column, weight--)) {
                weight = 0;
